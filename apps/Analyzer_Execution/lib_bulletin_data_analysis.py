...
def get_bulletin_field(field_name, field_info, field_separator=';',
                       field_value_null=None, field_time_format='%Y-%m-%d %H:%M'):

    if field_value_null is None:
        field_value_null = 'NA'
    ...
    if field_name in list(field_info.keys()):
        ...
        if isinstance(field_value, list):
            ...
            for elem_value in field_value:
                if isinstance(elem_value, pd.Timestamp):
                    elem_value = elem_value.strftime(field_time_format)
                tmp_value.append(elem_value)
            field_value = field_separator.join(tmp_value)
...
def create_bulletin_obj(run_name_list, run_description_list, run_time_list,
                        ...
                        view_warnings_list, view_execution_list):
    ...
    bulletin_obj = {
        ...
        'view_warnings': view_warnings_list, 'view_execution': view_execution_list}
...
def organize_bulletin_obj(section_dframe_generic, sep_root=';', sep_sub='::'):
    ...
    if not section_dframe_generic.empty:
        ...
        view_warnings = section_dframe_generic['view_warnings'].values[0]
        view_execution = section_dframe_generic['view_execution'].values[0]
        ...
        view_warnings, view_execution = False, False
...
def organize_bulletin_warnings_section(bulletin_dframe_generic, sections_dframe):
    ...
    view_warnings_summary, view_execution_summary = [], []
    ...
    for id_row, section_row in sections_dframe.iterrows():
        ...
        if not section_dframe_generic.empty:
            ...
            run_name_list, run_description_list, run_time_list, thr_data_list, thr_time_list, thr_run_list, \
                ...
                section_longitude, section_latitude, section_tag, section_name, section_code, section_catchment, \
                section_description, domain_description, \
                view_warnings, view_execution = organize_bulletin_obj(section_dframe_generic)
            ...
            view_warnings_summary.append(view_warnings)
            view_execution_summary.append(view_execution)
            ...
            log_stream.info(' -------> Section "' + section_reference.lower() +
                            ' ... FAILED. Datasets are not available for this section')

    # create bulletin obj
    bulletin_dframe_sections = create_bulletin_obj(
        run_name_summary, run_description_summary, run_time_summary,
    ...
    return bulletin_dframe_sections
# -------------------------------------------------------------------------------------


# -------------------------------------------------------------------------------------
# method to organize bulletin threshold(s) generic
def organize_bulletin_warnings_generic(run_name_obj, run_summary, section_collections,
    ...
    pnt_domain_name_list = []
    pnt_view_warnings_list, pnt_view_execution_list = [], []

    # iterate over run domain(s)
    for run_domain, run_name_list in run_name_obj.items():

        # iterate over run name(s)
        for run_id, run_name_step in enumerate(run_name_list):

            # info run start
            log_stream.info(' -------> Run "' + run_name_step + '" ... ')
            # get run data and info
            ...
            if run_data is not None:
                ...
                for id_step, (time_step, data_step) in enumerate(run_data.items()):
                    ...
                    run_view_warnings = run_info['view_warnings'][0]
                    run_view_execution = run_info['view_execution'][0]
                    ...
                    for pnt_id_step, pnt_name_step in enumerate(alert_section_list):
                        ...
                        if not pnt_info_step.empty:
                            ...
                            pnt_domain_name_list.append(run_domain)

                            pnt_view_warnings_list.append(run_view_warnings)
                            pnt_view_execution_list.append(run_view_execution)
    ...
    bulletin_dict = {
        ...
        'section_description': pnt_section_description_list,

        'domain_description': pnt_domain_description_list, 'domain_name': pnt_domain_name_list,

        'view_warnings': pnt_view_warnings_list, 'view_execution': pnt_view_execution_list}
...
def organize_bulletin_info(run_obj, run_summary,
                           tag_summary_data='data', tag_summary_info='info',
                           summary_no_data_run='NA', summary_no_data_time='NAT', summary_no_data_bool=False,
    ...
    run_last_list, run_period_list = [], []
    view_warn_list, view_exec_list = [], []

    # iterate over domain(s)
    for run_domain, run_name_list in run_obj.items():

        # info domain start
        log_stream.info(' ------> Domain "' + run_domain + '" ... ')
        ...
        for run_id_step, run_name_step in enumerate(run_name_list):
            ...
            run_last_step = get_bulletin_field('run_time_last', run_info_step, field_value_null=summary_no_data_time)
            run_period_step = get_bulletin_field(
                'run_time_period', run_info_step,
                field_separator=' :: ', field_value_null=summary_no_data_time)
            view_warn_step = get_bulletin_field('view_warnings', run_info_step, field_value_null=summary_no_data_bool)
            view_exec_step = get_bulletin_field('view_execution', run_info_step, field_value_null=summary_no_data_bool)
            ...
            run_period_list.append(run_period_step)
            view_warn_list.append(view_warn_step)
            view_exec_list.append(view_exec_step)

            # info run end
            log_stream.info(' -------> Run "' + run_name_step + '" ... DONE')

        # info domain end
        log_stream.info(' ------> Domain "' + run_domain + '" ... DONE')
    ...
    run_info_dict = {
        ...
        'run_last': run_last_list, 'run_period': run_period_list,
        'view_warnings': view_warn_list, 'view_execution': view_exec_list}
    ...
    # info start
    log_stream.info(' -----> Organize bulletin info warnings ... ')

    thr_run_name_ref, thr_run_tag_ref, thr_id_ref, thr_time_ref, thr_run_def, thr_default_ref = [], [], [], [], [], []
    thr_run_description_def, thr_code_def = [], []
    thr_warn_level_def, thr_warn_description_def = [], []
    thr_value_def, thr_time_def, thr_section_def = [], [], []
    thr_domain_def, thr_view_warnings, thr_view_execution = [], [], []
    ...
    for run_domain, run_name_list in run_obj.items():
        ...
        for run_id, run_name_step in enumerate(run_name_list):
            ...
            if run_data is not None:
                ...
                run_view_warnings = run_info['view_warnings'][0]
                run_view_execution = run_info['view_execution'][0]
                run_domain_reference = run_info['run_domain_reference'][0]
                ...
                for id_step, (time_step, data_step) in enumerate(run_data.items()):
                    ...
                    for idx in finite_idx:
                        ...
                        # merged information
                        thr_description_warn_string = ' :: '.join(
                            [alert_section_raw[idx], str(alert_index_raw[idx]), str(alert_value_raw[idx])])
                        thr_description_run_string = ' :: '.join([run_domain_reference, run_name_step])

                        thr_run_def.append(alert_run_select)
                        thr_section_def.append(alert_section_raw[idx])
                        thr_warn_level_def.append(summary_level_alert_status)
                        thr_code_def.append(summary_level_alert_code)

                        thr_domain_def.append(run_domain_reference)
                        thr_view_warnings.append(run_view_warnings)
                        thr_view_execution.append(run_view_execution)

                        thr_warn_description_def.append(thr_description_warn_string)
                        thr_run_description_def.append(thr_description_run_string)
                    ...
                    for idx in finite_idx:
                        ...
                        thr_warn_level_def.append(summary_level_alarm_status)
    ...
    run_thr_dict = {
        ...
        'data_warn_level': thr_warn_level_def, 'reference_run_description': thr_run_description_def,
        'data_code': thr_code_def, 'data_warn_description': thr_warn_description_def,
        'domain_reference': thr_domain_def,
        'view_warnings': thr_view_warnings, 'view_execution': thr_view_execution}

    run_thr_dframe = pd.DataFrame(run_thr_dict)

    # info end
    log_stream.info(' -----> Organize bulletin info warnings ... DONE')
