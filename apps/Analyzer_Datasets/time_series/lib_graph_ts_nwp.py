...
def plot_ts_discharge_nwp_probabilistic_only(
        file_name,
        file_attributes_nwp_probabilistic,
        df_rain_nwp_probabilistic,
        df_discharge_simulated_nwp_probabilistic,
        df_soil_moisture_nwp_probabilistic,
        df_discharge_observed=None,
        value_min_discharge=0, value_max_discharge=100,
        value_min_rain_avg=0, value_max_rain_avg=20,
        value_min_rain_accumulated=0, value_max_rain_accumulated=100,
    ...
    section_name, section_domain, \
        section_discharge_thr_alert, section_discharge_thr_alarm, \
        section_drained_area = get_ts_attrs(attrs_ts_nwp_prob)
    # Compute discharge axis limits
    axs_min_discharge, axs_max_discharge = set_ax_limits_discharge(
        max_value_dyn=section_discharge_thr_alarm,
        min_value_default=value_min_discharge, max_value_default=value_max_discharge)

    # Compute nwp probabilistic rain average
    df_rain_avg_probabilistic = compute_ts_ensemble_avg(df_rain_nwp_probabilistic, variable_axis=1)
    # Compute nwp probabilistic discharge average
    df_discharge_simulated_avg_probabilistic = compute_ts_ensemble_avg(df_discharge_simulated_nwp_probabilistic, variable_axis=1)
    # Compute nwp probabilistic soil moisture average
    ...
    if df_discharge_observed is None:
        df_values = deepcopy(df_discharge_simulated_avg_probabilistic.values)
        df_index = deepcopy(df_discharge_simulated_avg_probabilistic.index)
        df_values[:, 0] = -9996.0
        df_discharge_observed = pd.DataFrame(data=df_values, index=df_index)
    ...
    p11 = ax1.bar(df_rain_avg_probabilistic.index, df_rain_avg_probabilistic.values[:, 0],
                  color='#337ec9', alpha=1, width=0.025, align='edge')
    p12 = ax1.axvline(attrs_ts_nwp_prob['time_run'], color='#808080', linestyle='--', lw=2)
    ...
    p31 = ax3.plot(df_rain_avg_probabilistic.index, df_rain_avg_probabilistic.cumsum().values[:, 0],
                   color='#33A1C9', linestyle='-', lw=1)
    ...
    p21 = ax2.plot(df_discharge_observed.index, df_discharge_observed.values[:, 0],
                   color='#000000', linestyle='--', lw=1, marker='o', ms=4)
    p22 = ax2.plot(df_discharge_simulated_avg_probabilistic.index, df_discharge_simulated_avg_probabilistic.values[:, 0],
    ...
    p29 = ax2.axvline(attrs_ts_nwp_prob['time_run'], color='#000000', linestyle='--', lw=2, label='time run nwp')

    p210, p211 = None, None
    if (section_discharge_thr_alert is not None) and (section_discharge_thr_alert >= 0):
        p210 = ax2.axhline(section_discharge_thr_alert, color='#FFA500', linestyle='--',
                           linewidth=2, label=tag_discharge_thr_alert)
    if (section_discharge_thr_alarm is not None) and (section_discharge_thr_alarm >= 0):
        p211 = ax2.axhline(section_discharge_thr_alarm, color='#FF0000', linestyle='--',
                           linewidth=2, label=tag_discharge_thr_alarm)
    ...
    legend1 = ax2.legend((p21[0], p22[0], p41[0]),
                         (tag_discharge_obs_name,
                          tag_discharge_sim_name_nwp,
                          tag_soil_moisture_name),
                         frameon=False, ncol=2, loc=0)

    if (p210 is not None) and (p211 is not None):
        legend2 = ax2.legend((p24, p25, p210, p211),
